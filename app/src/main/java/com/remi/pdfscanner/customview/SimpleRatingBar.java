...
public class SimpleRatingBar extends View {
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor, ContextCompat.getColor(getContext(), R.color.white));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // draw stars
        if (gravity == Gravity.Left) drawFromLeftToRight(internalCanvas);
        else drawFromRightToLeft(internalCanvas);

        // draw view background color
        if (touchInProgress) canvas.drawColor(pressedBackgroundColor);
    ...
    private void drawStar(Canvas canvas, float x, float y, float filled, Gravity gravity) {
        ...
        if (drawBorderEnabled) canvas.drawPath(starPath, paintStarBorder);
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        if (isIndicator || (ratingAnimator != null && ratingAnimator.isRunning())) return false;
    ...
    private void setNewRatingFromTouch(float x, float y) {
        ...
        if (gravity != Gravity.Left) x = getWidth() - x;
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        if (builder.interpolator != null) ratingAnimator.setInterpolator(builder.interpolator);

        if (builder.animatorListener != null) ratingAnimator.addListener(builder.animatorListener);
    ...
    public class AnimationBuilder {
         ...
         * @return
         */
        public AnimationBuilder setInterpolator(BounceInterpolator interpolator) {
