...
public abstract class Sticker implements Serializable {
    ...
    private String url; //save uri image if load from storage
...
//        listSticker.add(this);
    }

    public Sticker(boolean createList) {
    }
    ...
    public void addToBackstack() {
        ...
        for (int i = listSticker.size()-1; i >0; i--) {
            if (i>currentPosition){
                listSticker.remove(i);
            }
        }
        while (listSticker.size()>20){
            listSticker.remove(0);
        }
        listSticker.add(this.createClone());
        currentPosition = listSticker.size() - 1;
    }
    ...
    public boolean canRedo() {
        if (listSticker==null)
            return false;
        return currentPosition < listSticker.size() - 1;
    }

    public abstract void undo();

    public abstract Sticker createClone();
    ...
    public abstract void draw(@NonNull Canvas canvas, float scale);
