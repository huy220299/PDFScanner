...
public class CropImageView extends ImageView {
    ...
    public IUndoCropImageView callbackUpdateUI;
    ...
    private static final int DEFAULT_LINE_COLOR = Color.parseColor("#5979FF");
    ...
    private List<Point[]> listPointHistory = new ArrayList<>();
    ...
    private Point[] mCropPoints; // 裁剪区域, 0->LeftTop, 1->RightTop， 2->RightBottom, 3->LeftBottom
    ...
    public boolean mAutoScanEnable = true;//是否进行自动识别边框
    ...
    /**
     * clear listPointHistory
     * add default mCropPoints to listPointHistory
     */
    public void setImageToCrop(Bitmap bmp) {
        setImageBitmap(bmp);
        currentScan = SmartCropper.scan(bmp);
        setCropPoints(mAutoScanEnable ? currentScan : null);
        listPointHistory.clear();
        listPointHistory.add(clonePoint(mCropPoints));
//        callbackUpdateUI.onUpdateUI();
        Log.e(TAG, "setImageToCrop: ");
    }

    public Point[] currentScan = null;
...
//        onDrawCusMagnifier(canvas);
    ...
    protected void onDrawPoints(Canvas canvas) {
        ...
        if (mShowEdgeMidPoint) {
            ...
            //中间锚点
            for (int i = 0; i < mEdgeMidPoints.length; i++) {
                Point point = mEdgeMidPoints[i];

                canvas.save();
                canvas.translate(getViewPointX(point), getViewPointY(point));
                if (i != 3) {
                    canvas.rotate(getDegrees(mEdgeMidPoints[i], mCropPoints[i + 1]), 0f, 0f);
                } else
                    canvas.rotate(getDegrees(mEdgeMidPoints[3], mCropPoints[0]), 0f, 0f);
                RectF rect = new RectF();

                rect.set(-dp2px(POINT_RADIUS) * 2, -dp2px(POINT_RADIUS) / 2, dp2px(POINT_RADIUS) * 2, dp2px(POINT_RADIUS) / 2);
                canvas.drawRoundRect(rect, dp2px(POINT_RADIUS), dp2px(POINT_RADIUS), mPointFillPaint);
                canvas.drawRoundRect(rect, dp2px(POINT_RADIUS), dp2px(POINT_RADIUS), mPointPaint);
                canvas.restore();
            }
...
//            }
    ...
    private float getDegrees(Point p1, Point p2) {
        float tan = (getViewPointY(p2) - getViewPointY(p1)) / (getViewPointX(p2) - getViewPointX(p1));
    ...
    //
    private void handleListHistoryPoint() {
        if (currentIndexPoint > -1) {
            while (listPointHistory.size() > currentIndexPoint + 1)
                listPointHistory.remove(listPointHistory.size() - 1);
        }

        listPointHistory.add(clonePoint(mCropPoints));
        currentIndexPoint = listPointHistory.size() - 1;
        if (callbackUpdateUI != null) callbackUpdateUI.onUpdateUI();
    }

    private Point[] clonePoint(Point[] input) {
        if (input == null) return null;
        Point[] newPoint = new Point[4];
        for (int i = 0; i < 4; i++) {
            newPoint[i] = new Point(input[i].x, input[i].y);
        }
        return newPoint;
    }
    ...
    public void actionNext() {
        currentIndexPoint++;
        setCropPoints(clonePoint(listPointHistory.get(currentIndexPoint)));
    }
    ...
    /**
     * check can back action
     */
    public boolean canPrevious() {
        return currentIndexPoint > 0 && currentIndexPoint < listPointHistory.size();
    }

    /**
     * check can next action
     */
    public boolean canNext() {
        return currentIndexPoint >= 0 && currentIndexPoint < listPointHistory.size() - 1;
    }
